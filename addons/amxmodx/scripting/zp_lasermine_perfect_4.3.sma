...
#define LM_COST 5 // Price of lasermine in a extra item menu
...
new const LM_MODEL[] = "models/ch2012/v_laser_mine.mdl"
...
public Remove_Lasermine(id) {
	...
	if(equal(EntityName, Lasermine_Classname) && pev(tgt,LASERMINE_OWNER) == id) {
		...
		g_lm_ammo[id]++;
		#endif
		emit_sound(id, CHAN_ITEM, Lasermine_Sounds[0], VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
...
PlaySound(i_Ent, i_SoundType) {
	...
	switch (i_SoundType) {
		case POWERUP_SOUND : {
			...
			emit_sound(i_Ent, CHAN_BODY , Lasermine_Sounds[0], 0.2, ATTN_NORM, 0, PITCH_NORM);
		}
		case ACTIVATE_SOUND: emit_sound(i_Ent, CHAN_VOICE, Lasermine_Sounds[0], 0.5, ATTN_NORM, 1, 75);
		case STOP_SOUND : {
			emit_sound(i_Ent, CHAN_BODY , Lasermine_Sounds[0], 0.2, ATTN_NORM, SND_STOP, PITCH_NORM);
			emit_sound(i_Ent, CHAN_VOICE, Lasermine_Sounds[0], 0.5, ATTN_NORM, SND_STOP, 75);
		}
	}
...
Show_Lasermine_Line(i_Ent,const Float:v_Origin[3], const Float:v_EndOrigin[3]) {
	...
	switch(g_lasermine_id[SPRITE][id]) {
		...
		case 1:	sprid = beam[0], sprwave = 5, fps = 1	// Shock
		case 2:	sprid = beam[0], sprwidth = 25, sprspd = 20	// Neon
		case 3:	sprid = beam[0], sprwidth = 15, frame = 1	// Dotted
		case 4:	sprid = beam[0], sprwidth = 40, sprspd = 15, frame = 2	// 4i20
		case 5:	sprid = beam[0], sprwidth = 8, frame = 3	// Triangle
		case 6:	sprid = beam[0], frame = 4					// Double Ray
		case 7:	sprid = beam[0], sprwidth = 15, frame = 5, sprspd = 20	// Spiral 
		case 8:	sprid = beam[0], sprwidth = 30, sprspd = 10, frame = 6	// Heartbeat 
		case 9:	sprid = beam[0], sprwidth = 30, sprspd = 10, frame = 7	// Heart Love
		case 10: sprid = beam[0], sprwidth = 30, sprspd = 10, frame = 8 // Amazing Star
		case 11: sprid = beam[0], sprwidth = 40, sprspd = 15, frame = 9 // Skull
	}
...
public Lasermine_Damage(iCurrent,isHit) {
	...
	if(is_user_connected(isHit)) {
		if(is_user_alive(isHit) && zp_get_user_zombie(isHit) && !g_lasermine_imune[isHit]) {
			emit_sound(isHit, CHAN_WEAPON, Lasermine_Sounds[0], 1.0, ATTN_NORM, 0, PITCH_NORM)
...
stock get_skin_name(id, skin)
{
	...
	formatex(szSkinLang, charsmax(szSkinLang), "%L", id, skin_langs[modelid][szSkin1])
	return szSkinLang
}
