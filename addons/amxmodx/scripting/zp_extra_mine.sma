...
public GiveTripmineToHumans()
{
	for(new id = 1; id <= MaxClients; id++)
	{
		...
		{
			iPlayerTripmine[id]++
			client_print(id, print_chat, "[LaserMine] You received a free Lasermine! Press E to plant")
			client_print(id, print_center, "You received a Lasermine!")
		}
	}
}
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	if(buttons & IN_USE && !(oldbuttons & IN_USE))
	{
		...
		{
			client_print(id, print_chat, "[LaserMine] You can only have %d lasermine planted at a time!", get_pcvar_num(CvarMaxPlant))
			return FMRES_IGNORED
		}
		if(!IsOnWall(id))
		{
			client_print(id, print_chat, "[LaserMine] LaserMine must be planted on a wall.")
			return FMRES_IGNORED
		}
	...
	else if(buttons & IN_USE && g_IsPlanting[id])
	{
		...
		if(elapsed >= PLANT_TIME)
		{
			...
			CreateTripmine(id)
			client_print(id, print_chat, "[LaserMine] You successfully planted lasermine, once it explodes, you can place another one")
		}
	}
...
public zp_fw_items_select_post(id, item, igcost)
{
	...
	iPlayerTripmine[id]++
	if(iPlayerTripmine[id] == 1) client_print(id, print_chat, "[LaserMine] You Bought Lasermine! Press E to plant");
...
CreateTripmine(id)
{
	...
	{
		client_print(id, print_chat, "[TRIPMINE] Target origin is too close with a door@tripmine.")
		return;
	}
