...
new text[100]
format(text,99,"^x04[ZP]^x01 schopnost je pripravena")
...
show_hudmessage(id, "Cooldown: %d",is_cooldown_time[id])
}else{
remove_task(id)
}
}
...
public zp_user_infected_post(id, infector)
{
	if ((zp_get_user_zombie_class(id) == g_zclass_tank) && !zp_get_user_nemesis(id))
	{
		...
		format(text,99,"^x04[ZP]^x01 Schopnost %d ^x",note_cooldown)
		message_begin(MSG_ONE,get_user_msgid("SayText"),{0,0,0},id)
		write_byte(id)
		write_string(text)
		message_end()
