...
public zprava(id)
{
	...
	ChatColor(id, "!g[Crawler] !gMas obrovsky damage! Davas kriticke kousnuti");
	ChatColor(id, "!g[Crawler] !gMas obrovsky damage! Davas kriticke kousnuti");
}
...
{	
	...
	if( zp_get_user_zombie_class( attacker ) == g_zclass_crab)  // Hr�� mus� b�t Crab zombie
	{
		if( random_num( 0, 100 ) < random_num( 0, 100 ) < random_num( 0, 100 ))
		{
			...
			cmd_fade8( attacker )
			client_print( attacker, print_center, "Kriticke kousnuti!")
			
			cmd_fade9( victim )
			client_print( victim, print_center, "Kriticke kousnuti!")
		}
	}
