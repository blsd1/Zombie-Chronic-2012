...
new const GhostLaugh[] = { "sound/ch2012/ghost1.wav" }
new const GhostVisible[] = { "sound/ch2012/ghost_nactive.wav" }
new const GhostInvisible[] = { "sound/ch2012/ghost_active.wav" }
...
public ShowHUDstealthes(id)
{
	if(is_valid_ent(id) && is_user_alive(id))
	{
		...
		show_hudmessage(id, "Invisibility: %d s", i_stealth_time_hud[id])
	}else{
		remove_task(id)
	}
}
...
public zp_user_infected_post(id, infector)
{
	if ((zp_get_user_zombie_class(id) == g_zclass_ghost) && !zp_get_user_nemesis(id))
	{
		...
		zp_override_user_idlesound(id, "ch2012/ghost_start1.wav", sizeof(g_spawn_sound))
