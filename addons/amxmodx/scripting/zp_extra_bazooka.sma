...
public event_HLTV()
{
	...
	for( new id = 1; id <= 32; id++ )
	{
		if(has_baz[id] && is_user_connected(id) && is_user_alive(id))
		{
			...
			client_print(id, print_chat, "[Bazooka] Cooldown 60 sekund na zaciatku kola!")
		}
		else
		{
			has_baz[id] = false
...
public zp_extra_item_selected(player, itemid)
{
	if (itemid == g_itemid)
	{
		if ( has_baz[player] )
		{
			...
			client_print(player, print_chat, "Uz mas raketu ty kokot.");
		}
		else {
			has_baz[player] = true;
			...
			ChatColor(player, "!t[DarTom] !gLave tlacitko mysi - Strelba");
...
public hud_prebijeni(id)
{
	if(prebijeci_doba[id] > 1)
	{
		...
		show_hudmessage(id, "Nabijanie Bazooky: %d s",prebijeci_doba[id])
	}
}
...
public rpg_reload(id)
{
	...
	if ( get_user_weapon(id) == CSW_KNIFE ) switch_to_knife(id);
	{
		CanShoot[id] = true
                remove_task( id );
		client_print(id, print_center, "Raketa Nabita!")
