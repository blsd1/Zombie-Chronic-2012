...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type)
{
	...
	{
		// Kill the victim instead of infecting
	...
	if (!g_freezetime)
	{
		...
		if (target > 0 && target <= g_maxplayers && g_isalive[target] && target != id)
		{
			...
			if (g_zombie[id] && g_zombie[target])
			{
				...
				set_hudmessage(200, 100, 0, -1.0, 0.55, 1, 0.5, 0.5, 0.0, 0.0, 4)
				ShowSyncHudMsg(id, CreateHudSyncObj(), "^n%s^nHP: %d^nAmmopacks: %d^n", 
			...
			else if (!g_zombie[id] && !g_zombie[target])
			{
				...
				set_hudmessage(0, 100, 200, -1.0, 0.55, 1, 0.5, 0.5, 0.0, 0.0, 4)
				ShowSyncHudMsg(id, CreateHudSyncObj(), "^n^n%s^n^nHP: %d^nAmmopacks: %d^n", 
					name, pev(target, pev_health), g_ammopacks[target])
			}
		}
	}
...
show_menu_game(id)
{
	...
	if (g_isalive[id] && !g_zombie[id] && !g_survivor[id])
		len += formatex(menu[len], charsmax(menu) - len, "\r1.\w Custom Weapons \y[$]^n", id)
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d1. Custom Weapons [$]^n", id)
	...
	// 4. Epic Menu
	if (g_isalive[id])
		len += formatex(menu[len], charsmax(menu) - len, "\r4.\w Epic Menu \y[Spirits: %d]^n", g_spirits[id])
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d4. Epic Menu [Spirits: %d]^n", g_spirits[id])
...
show_menu_epic(id)
{
	...
	// 1. Ultimate Damage (25% damage + 25% HP kill bonus) - Cost: 5 spirits
	if (g_ultimate_damage[id])
		len += formatex(menu[len], charsmax(menu) - len, "\d1. Ultimate Damage \y[ACTIVE]^n")
	else if (g_spirits[id] >= 5)
		len += formatex(menu[len], charsmax(menu) - len, "\r1.\w Ultimate Damage \y(+25%% DMG, +25%% HP Kill) \r[5 Spirits]^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d1. Ultimate Damage (+25%% DMG, +25%% HP Kill) [5 Spirits]^n")
	
	// 0. Exit
	len += formatex(menu[len], charsmax(menu) - len, "^n^n\r0.\w Zavriet")
...
public menu_epic(id, key)
{
	...
	switch (key)
	{
		case 0: // Ultimate Damage
		{
			// Already active?
			if (g_ultimate_damage[id])
			{
				zp_colored_print(id, "^x04[ZP]^x01 Ultimate Damage je uz aktivny!")
				show_menu_epic(id)
				return PLUGIN_HANDLED;
			}
			...
			if (g_spirits[id] < 5)
			{
				zp_colored_print(id, "^x04[ZP]^x01 Nemas dostatok Spirits! Potrebujes 5.")
				show_menu_epic(id)
				return PLUGIN_HANDLED;
			}
			...
			g_spirits[id] -= 5
			g_ultimate_damage[id] = true
			zp_colored_print(id, "^x04[ZP]^x01 Ultimate Damage aktivovany! ^x04+25%% damage^x01 a ^x04+25%% HP^x01 za kill!")
		}
	}
...
make_a_zombie(mode, id)
{
	...
	else
	{
		...
		else
		{
			...
			set_hudmessage(200, 128, 0, HUD_EVENT_X, HUD_EVENT_Y, 0, 0.0, 5.0, 1.0, 1.0, -1)
...
zombieme(id, infector, nemesis, silentmode, rewards)
{
	...
	if (!silentmode)
	{
		...
		else
		{
			...
			set_hudmessage(255, 0, 0, -1.0, 0.22, 0, 0.0, 5.0, 1.0, 1.0, -1)
...
infection_effects(id)
{
	...
	if (!g_frozen[id])
	{
		...
		message_begin(MSG_ONE, g_msgScreenFade, _, id)
		write_short(UNIT_SECOND * 2) // duration - 2 seconds fadeout
		write_short(UNIT_SECOND * 3) // hold time - 3 seconds
		write_short(FFADE_IN) // fade type - fade to black
