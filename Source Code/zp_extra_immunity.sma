...
public zp_extra_item_selected( id, itemid )
{
	if(itemid == g_itemID)
	{
		...
		if ( Limit[ id ] == 1 )
		{
			client_print( id, print_center, "%L", id, "USED_THIS_ROUND" );
			zp_set_user_ammo_packs( id, zp_get_user_ammo_packs( id ) + ITEM_COST );
		...
		{
			zp_set_user_ammo_packs( id, zp_get_user_ammo_packs( id ) + ITEM_COST );
			client_print(id, print_center, "%L", id, "NEED_ZOMBIES")
			return;
		}	
...
public CountDown( id )
{
	...
	{		
		...
		client_print( id, print_center, "%L", id, "GONE" );
	...
	client_print(id, print_center, "%L", id, "REMAINING", Time[id]);
