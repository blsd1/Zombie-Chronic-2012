			...
			if ((iButtons & IN_RELOAD) && !(iOldButtons & IN_RELOAD))
			{
				...
				if (g_pritiahnutia[id] != 0)
				{
					if (hook[id])
					{
						...
						g_pritiahnutia[id]--
						remove_task(id)
						ChatColor(id, "!t[Spider zombie] !g%L", id, "SPIDER_USED", 3 - g_pritiahnutia[id])
						...
						if (g_pritiahnutia[id] == 0)
						{
							...
							ChatColor(id, "!t[Spider zombie] !g%L", id, "WEBS_OUT")
						}
					}
					else
					{
				...
				{
					ChatColor(id, "!t[Spider zombie] !g%L", id, "CANT_PULL", g_cooldown_time[id])
				}
			}
...
public restore_hooks(id)
{
	...
	if (zp_get_user_zombie(id) && zp_get_user_zombie_class(id) == g_zclass_spider && !zp_get_user_nemesis(id))
	{
		...
		g_cooldown_time[id] = 0
		remove_task(id + 20000)
		ChatColor(id, "!t[Spider zombie] !g%L", id, "HOOKS_RESTORED")
	}
}
