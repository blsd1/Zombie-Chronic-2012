...
public plugin_natives()
{
	...
	register_native("zp_remove_user_chainsaw", "native_remove_user_chainsaw", 1)
}
	...
	if (itemid == g_iItemID)
	{
		...
		if(get_pcvar_num(cvar_enable))
		{
			...
			if (g_iHasChainsaw[id])
			{
				...
				client_printcolor(id, "/g[ZP]/y %L", id, "CHAINSAW_ALREADY")
			}
			else 
			{
				// Boolean
				...
				// Client Print
				client_printcolor(id, "/g[ZP]/y %L", id, "CHAINSAW_RECEIVED")
		...
		{
			zp_set_user_ammo_packs(id, zp_get_user_ammo_packs(id) + chainsaw_ap_cost)
			client_printcolor(id, "/g[ZP]/y %L", id, "CHAINSAW_DISABLED")
		}
	}
}
...
public native_remove_user_chainsaw(id)
{
	if (!is_user_connected(id))
	{
		log_error(AMX_ERR_NATIVE, "[CHAINSAW] Invalid Player (%d)", id)
	...
	// Take the chainsaw away and restore the normal knife
	g_iHasChainsaw[id] = false
