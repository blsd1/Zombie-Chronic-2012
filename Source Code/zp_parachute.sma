...
// Remove parachute when player gets infected
...
// Give parachute when player gets cured (becomes human again)
...
public Ham_CBasePlayer_Spawn_Post(id) {
	...
	else if( HasUserParachute(id) ) {
		// Remove parachute if player is zombie
		new iEnt = g_iUserParachute[id]
		if( iEnt ) {
			RemoveUserParachute(id, iEnt)
		}
...
public fw_Start(id) {
	// Only allow humans to use parachute
	if( ~HasUserParachute(id) || !is_user_alive(id) || zp_get_user_zombie(id) ) {
		return
	}
